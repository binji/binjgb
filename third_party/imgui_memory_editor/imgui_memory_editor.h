...
struct MemoryEditor
{
    ...
    u8              (*ReadFn)(const u8* data, size_t off, void* user_data);  // = NULL   // optional handler to read bytes.
    void            (*WriteFn)(u8* data, size_t off, u8 d, void* user_data); // = NULL   // optional handler to write bytes.
    bool            (*HighlightFn)(const u8* data, size_t off, void* user_data);//NULL   // optional handler to return Highlight property (to support non-contiguous highlighting).
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
            {
                ...
                if (DataEditingAddr == addr)
                {
                    ...
                    if (DataEditingTakeFocus)
                    {
                        ...
                        sprintf(DataInputBuf, format_byte, ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr]);
                    }
                    ...
                    sprintf(user_data.CurrentBufOverwrite, format_byte, ReadFn ? ReadFn(mem_data, addr, this->UserData) : mem_data[addr]);
                    ...
                    {
                        if (WriteFn)
                            WriteFn(mem_data, addr, (u8)data_input_value, this->UserData);
                        else
