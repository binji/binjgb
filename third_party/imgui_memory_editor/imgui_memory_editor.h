// -- Modified to pass UserData to callbacks
//
...
struct MemoryEditor
{
    ...
    u8              (*ReadFn)(u8* data, size_t off, void* user_data);        // = NULL   // optional handler to read bytes
    void            (*WriteFn)(u8* data, size_t off, u8 d, void* user_data); // = NULL   // optional handler to write bytes
    bool            (*HighlightFn)(u8* data, size_t off, void* user_data);   // = NULL   // optional handler to return Highlight property (to support non-contiguous highlighting)
    void*           UserData;                                                // = NULL   // optional data passed to callbacks
    ...
    void DrawContents(u8* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
            {
                ...
                // Draw highlight
                if ((addr >= HighlightMin && addr < HighlightMax) || (HighlightFn && HighlightFn(mem_data, addr, UserData)))
                {
                    ImVec2 pos = ImGui::GetCursorScreenPos();
                    float highlight_width = s.GlyphWidth * 2;
                    bool is_next_byte_highlighted =  (addr + 1 < mem_size) && ((HighlightMax != (size_t)-1 && addr + 1 < HighlightMax) || (HighlightFn && HighlightFn(mem_data, addr + 1, UserData)));
                ...
                if (DataEditingAddr == addr)
                {
                    ...
                    if (DataEditingTakeFocus)
                    {
                        ...
                        sprintf(DataInputBuf, "%02X", ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr]);
                    }
                    ...
                    struct InputUserData
                    {
                        ...
                        {
                            InputUserData* user_data = (InputUserData*)data->UserData;
                    ...
                    InputUserData user_data;
                    user_data.CursorPos = -1;
                    sprintf(user_data.CurrentBufOverwrite, "%02X", ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr]);
                    ImGuiInputTextFlags flags = ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_NoHorizontalScroll | ImGuiInputTextFlags_AlwaysInsertMode | ImGuiInputTextFlags_CallbackAlways;
                    if (ImGui::InputText("##data", DataInputBuf, 32, flags, InputUserData::Callback, &user_data))
                    ...
                    {
                        if (WriteFn)
                            WriteFn(mem_data, addr, (u8)data_input_value, UserData);
                        else
                ...
                else
                {
                    ...
                    u8 b = ReadFn ? ReadFn(mem_data, addr, UserData) : mem_data[addr];
