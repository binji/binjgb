...
struct MemoryEditor
{
    ...
    ImU8            (*ReadFn)(const ImU8* data, size_t off, void* user_data);  // = NULL   // optional handler to read bytes.
    void            (*WriteFn)(ImU8* data, size_t off, ImU8 d, void* user_data); // = NULL   // optional handler to write bytes.
    bool            (*HighlightFn)(const ImU8* data, size_t off, void* user_data);//NULL   // optional handler to return Highlight property (to support non-contiguous highlighting).
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
            {
                ...
                if (DataEditingAddr == addr)
                {
                    ...
                    if (DataEditingTakeFocus)
                    {
                        ...
                        sprintf(DataInputBuf, format_byte, ReadFn ? ReadFn(mem_data, addr, FnUserData) : mem_data[addr]);
                    }
                    struct UserData
                    {
                    ...
                    sprintf(user_data.CurrentBufOverwrite, format_byte, ReadFn ? ReadFn(mem_data, addr, FnUserData) : mem_data[addr]);
