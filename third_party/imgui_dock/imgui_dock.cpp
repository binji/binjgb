// based on https://bitbucket.org/duangle/liminal/src/tip/src/liminal/imgui_dock.cpp
...
struct DockContext {
  ...
  struct Dock {
    ...
    Dock& getRoot() {
      Dock* dock = this;
      while (dock->parent) dock = dock->parent;
      return *dock;
    }
  ...
  ImGuiDockSlot m_next_dock_slot = ImGuiDockSlot_Tab;
  ...
  void handleDrag(Dock& dock) {
    ...
    Begin("##Overlay", NULL, ImVec2(0, 0), 0.f,
  ...
  void doUndock(Dock& dock) {
    ...
    if (container) {
      ...
      if (remove_container) {
        ...
        if (container == m_next_parent) m_next_parent = nullptr;
        container->~Dock();
        MemFree(container);
      }
    }
  ...
  void debugWindow() {
    ...
    if (Begin("Dock Debug Info")) {
      for (int i = 0; i < m_docks.size(); ++i) {
        if (TreeNode((void*)(uintptr_t)i, "Dock %d (%p)", i, m_docks[i])) {
          Dock& dock = *m_docks[i];
...
void SetNextDock(ImGuiDockSlot slot) { g_dock.m_next_dock_slot = slot; }

void SetNextDockParentToRoot() { g_dock.m_next_parent = nullptr; }
...
void DockDebugWindow() { g_dock.debugWindow(); }
